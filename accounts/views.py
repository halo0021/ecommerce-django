...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            phone_nuber=form.cleaned_data['phone_nuber']
            email=form.cleaned_data['email']
            password=form.cleaned_data['password']

            #la base de datos  pide el user name y se generara mediante  el correo electronico halo@hotmail.es
            #capturando el primer segmento de  correo  en este caso seria halo
            username = email.split("@")[0]
            # 4 horas  buscando el erro y era una coma 
            user = Account.objects.create_user(first_name=first_name, last_name=last_name, email=email, username=username, password=password)
            user.phone_nuber = phone_nuber
            user.save()
            ...
            mail_subject='Por favor activa  la cuenta en la tienda'
            #contenido del correo electronico que le llegara al cliente
            body= render_to_string('accounts/account_verification_email.html', {
                'user':user,
                'domain':current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),#tranforma  en colecion de caracteres hexadecimal
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(request, 'Se a iniciado sesion exitosamente')
            url = request.META.get('HTTP_REFERER') #http://localhost:8000/accounts/login/?next=/cart/checkout/ SE CAPTURA LA URL
            try:
...
#login sera requerido mediante la etiqueta
